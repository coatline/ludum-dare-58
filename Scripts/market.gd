...
var histories = [
	...
	MarketHistory.new(self, 7, 1440),
	MarketHistory.new(self, 30, 1440),
	MarketHistory.new(self, 365, 1440)
]
...
func get_graph(target_past_minutes: float, x_scale: float, y_scale: float, max_price: float) -> PackedVector2Array:
	...
	var target_past_ticks = target_past_minutes * Economy.TICKS_PER_GAME_MINUTE
	var history_scale = 0
	var history: MarketHistory = histories[history_scale]
	
	while target_past_minutes > history.record_interval_minutes * history.max_record_minutes:
		history_scale += 1
	...
	var max_ticks = (target_past_minutes / (history.record_interval_minutes)) * Economy.TICKS_PER_GAME_MINUTE
